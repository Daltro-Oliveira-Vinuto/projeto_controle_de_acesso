from django.urls import path
from rest_framework.routers import DefaultRouter
from rest_framework_nested import routers as nested_routers

from .views import EstudanteViewSet, DigitalViewSet, ImportarEstudantesView, VerificarDigitalView

# ─────────────────────────────────────────────────────────────
# Router principal — cria:
# /estudantes/
# /estudantes/{id}/
# ─────────────────────────────────────────────────────────────
router = DefaultRouter()
router.register(r'estudantes', EstudanteViewSet, basename='estudantes')

# ─────────────────────────────────────────────────────────────
# Router aninhado — cria:
# /estudantes/{estudante_pk}/digitais/
# ─────────────────────────────────────────────────────────────
estudante_router = nested_routers.NestedDefaultRouter(
    router, r'estudantes', lookup='estudante'
)
estudante_router.register(
    r'digitais',
    DigitalViewSet,
    basename='estudante-digitais'
)

# ─────────────────────────────────────────────────────────────
# URLs finais
# ─────────────────────────────────────────────────────────────
urlpatterns = [
    # ROTAS DO ROUTER
    *router.urls,
    *estudante_router.urls,

    # IMPORTAÇÃO CSV (CORRIGIDO — SEM CONFLITO)
    path(
        'importar-estudantes/',
        ImportarEstudantesView.as_view(),
        name='importar-estudantes'
    ),
    path(
        'verificar-digital/',
        VerificarDigitalView.as_view(),
        name='verificar-digital'
    ),
]